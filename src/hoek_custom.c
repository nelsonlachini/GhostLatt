				...
				for(nmu[3]=0 ; nmu[3]<N_ins ; nmu[3]++){
					for(mu=0 ; mu<4 ; mu++){
							...
							if( ((nmu[0]==N_ins-1)) || ((nmu[1]==N_ins-1)) || ((nmu[2]==N_ins-1)) || ((nmu[3]==N_ins-1)) ){
								...
								mmprodv(aux, getLink(lattice,nmu[0]+x0[0],nmu[1]+x0[1],nmu[2]+x0[2],nmu[3]+x0[3],mu), getLink(lattice,xv[0]+x0[0],xv[1]+x0[1],xv[2]+x0[2],xv[3]+x0[3],mu) );
								su2sroot(getLink(lattice,nmu[0]+x0[0],nmu[1]+x0[1],nmu[2]+x0[2],nmu[3]+x0[3],mu) , aux);
						}
					}
				}
			}
...
double updateStapleBorder(double * lattice , int t, int x, int y, int z, int mi , double * _staple, double * x0, int N_ins){
	...
	for(ni=0 ; ni<4 ; ni++){
		if(ni != mi){
			...
			copyv(aux1 , getLink( lattice, x0[0]+getStepBorder(t,ami[0],N_ins) , x0[1]+getStepBorder(x,ami[1],N_ins) , x0[2]+getStepBorder(y,ami[2],N_ins) , x0[3]+getStepBorder(z,ami[3],N_ins), ni) );
			hermcv( aux2 , getLink( lattice, x0[0]+getStepBorder(t,ani[0],N_ins) , x0[1]+getStepBorder(x,ani[1],N_ins) , x0[2]+getStepBorder(y,ani[2],N_ins) , x0[3]+getStepBorder(z,ani[3],N_ins) , mi) );
			...
			hermcv(aux1 , getLink( lattice, x0[0]+getStepBorder(t,ami[0]-ani[0],N_ins), x0[1]+getStepBorder(x,ami[1]-ani[1],N_ins) , x0[2]+getStepBorder(y,ami[2]-ani[2],N_ins) , x0[3]+getStepBorder(z,ami[3]-ani[3],N_ins) , ni));
			hermcv(aux2 ,getLink( lattice, x0[0]+getStepBorder(t,-ani[0],N_ins) , x0[1]+getStepBorder(x,-ani[1],N_ins) , x0[2]+getStepBorder(y,-ani[2],N_ins) , x0[3]+getStepBorder(z,-ani[3],N_ins) , mi) );
			copyv( aux3 , getLink( lattice, x0[0]+getStepBorder(t,-ani[0],N_ins) , x0[1]+getStepBorder(x,-ani[1],N_ins) , x0[2]+getStepBorder(y,-ani[2],N_ins) , x0[3]+getStepBorder(z,-ani[3],N_ins) , ni));
				...
				for(z=0 ; z<N_ins ; z++){
					for(mi=0 ; mi<4 ; mi++){
						if( ((t<inf) || (t>sup)) || ((x<inf) || (x>sup)) || ((y<inf) || (y>sup)) || ((z<inf) || (z>sup)) ){
							...
							cmprodv( getLink(lattice,t+x0[0],x+x0[1],y+x0[2],z+x0[3],mi), 1e0/sqrt(detv(staple)) , staple);
						}
					}
				}
			}
			...
			for(y=0 ; y<N_ins ; y++){
				for(z=0 ; z<N_ins ; z++){
					for(mu=0 ; mu<4 ; mu++){
						...
						//getchar();
						umuregV(getLink(U,t+x0[0],x+x0[1],y+x0[2],z+x0[3],mu),t-c0,x-c0,y-c0,z-c0,mu,partition_number,R,N_ins);
						singtrans(getLink(U,t+x0[0],x+x0[1],y+x0[2],z+x0[3],mu),getLink(U,t+x0[0],x+x0[1],y+x0[2],z+x0[3],mu),t-c0,x-c0,y-c0,z-c0,mu,N_ins);
					}
				}
			}
		}
