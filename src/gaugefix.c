			...
			for(y=0 ; y<N ; y++){
				for(z=0 ; z<N ; z++){
					for(mi=0 ; mi<4 ; mi++){
						...
						mmprodv(auxU , getU(U,t,x,y,z,mi) , auxU);
			...
			for(y=0 ; y<N ; y++){
				for(z=0 ; z<N ; z++){
					for(mi=0 ; mi<4 ; mi++){
						...
						setquadv(&ami[0],mi);
						hermcv(auxU , getg(g, getStepT(t,+ami[0]) , getStep(x,+ami[1]) , getStep(y,+ami[2]) , getStep(z,+ami[3])) );
						mmprodv(auxU , getU(Uin,t,x,y,z,mi) , auxU);
						mmprodv(getU(Uout,t,x,y,z,mi) , getg(g,t,x,y,z) , auxU);
					}
				}
			}
		}
			...
			for(y=0;y<N;y++){
				for(z=0;z<N;z++){
					for(mi=0;mi<4;mi++){
						...
						sumv(aux1, getU(U,t,x,y,z,mi) , aux1);
						hermcv(aux2,aux1);
			...
			for(y=0 ; y<N ; y++){
				for(z=0 ; z<N ; z++){
					...
					for(mi=0 ; mi<4 ; mi++){
						...
						hermcv(aux1, getU(U,t,x,y,z,mi));
						sumv(aux1, aux1, getU(U , getStepT(t,-ami[0]) , getStep(x,-ami[1]) , getStep(y,-ami[2]) , getStep(z,-ami[3]) ,mi));
						sumv(h,h,aux1);

						hermcv(aux2, getU(U , getStepT(t,-ami[0]) , getStep(x,-ami[1]) , getStep(y,-ami[2]) , getStep(z,-ami[3]) ,mi));
						sumv(aux2,aux2,getU(U,t,x,y,z,mi));
						cmprodv(aux2, -1, aux2);
						sumv(h,h,aux2);
						//hermcv( aux1 , getU(U , getStepT(t,-ami[0]) , getStep(x,-ami[1]) , getStep(y,-ami[2]) , getStep(z,-ami[3]) ,mi) );
						//hermcv( aux2 , getg(g, getStepT(t,-ami[0]) , getStep(x,-ami[1]) , getStep(y,-ami[2]) , getStep(z,-ami[3])) );
...
double calibrate_stoc(double * lattice , double tol){
	...
	double * g = malloc(sizeof(double)*totalV*4);
	double * latticetemp = malloc(sizeof(double)*dimLattice);
